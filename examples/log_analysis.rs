...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !connectivity.indexer_connected {
        warn!("⚠️  Cannot connect to Wazuh Indexer: {}", 
              connectivity.indexer_error.as_deref().unwrap_or("Unknown error"));
        ...
        Ok(analysisd_stats) => {
            ...
            if analysisd_stats.events_received > 0 {
                let processing_rate = (analysisd_stats.events_processed as f64 / 
                                     analysisd_stats.events_received as f64) * 100.0;
                let alert_rate = (analysisd_stats.alerts_written as f64 / 
                                analysisd_stats.events_processed as f64) * 100.0;
    ...
    match logs_client.get_remoted_stats().await {
        Ok(remoted_stats) => {
            ...
            if remoted_stats.evt_count > 0 {
                let avg_message_size = remoted_stats.recv_bytes as f64 / 
                                     remoted_stats.evt_count as f64;
                println!("   Average Message Size: {:.1} bytes", avg_message_size);
            }
            
            if remoted_stats.discarded_count > 0 {
                let discard_rate = (remoted_stats.discarded_count as f64 / 
                                  remoted_stats.evt_count as f64) * 100.0;
        ...
        match logs_client.get_logcollector_stats(&agent.id).await {
            Ok(logcollector_stats) => {
                ...
                if logcollector_stats.events > 0 {
                    let avg_event_size = logcollector_stats.bytes as f64 / 
                    ...
                    if logcollector_stats.events_dropped > 0 {
                        let drop_rate = (logcollector_stats.events_dropped as f64 / 
                                       logcollector_stats.events as f64) * 100.0;
            ...
            Err(e) => warn!("Failed to get log collector stats for agent {}: {}", agent.id, e),
        }
    }
    ...
    match logs_client.get_manager_logs(Some(20), None, None, None, None).await {
        Ok(manager_logs) => {
        ...
        Ok(hourly_stats) => {
            ...
            if let Ok(json_value) = serde_json::to_value(&hourly_stats) {
                if let Some(data) = json_value.get("data") {
                    if let Some(affected_items) = data.get("affected_items").and_then(|v| v.as_array()) {
                        if let Some(hourly_data) = affected_items.first() {
                            if let Some(averages) = hourly_data.get("averages").and_then(|v| v.as_array()) {
                                let interactions = hourly_data.get("interactions")
                                ...
                                let max_value = averages.iter()
                                    .filter_map(|v| v.as_u64())
                                    .max().unwrap_or(0);
                                let peak_hours: Vec<usize> = averages.iter()
                                    .enumerate()
                                    .filter_map(|(i, v)| {
                                        if v.as_u64() == Some(max_value) { Some(i) } else { None }
                                    })
                                    .collect();
                                
                                if !peak_hours.is_empty() {
                                    println!("   🔥 Peak Activity Hours: {}:00 ({})", 
                                        peak_hours.iter().map(|h| format!("{:02}", h)).collect::<Vec<_>>().join(", "),
                                        max_value);
                                }
                            }
                        }
                    ...
                    if let Some(total_affected) = data.get("total_affected_items").and_then(|v| v.as_u64()) {
            ...
            if let Ok(json_value) = serde_json::to_value(&weekly_stats) {
                if let Some(data) = json_value.get("data") {
                    if let Some(affected_items) = data.get("affected_items").and_then(|v| v.as_array()) {
                        ...
                        for day_data in affected_items {
                            for day_name in &days {
                                if let Some(day_info) = day_data.get(day_name) {
                                    if let Some(hours) = day_info.get("hours").and_then(|v| v.as_array()) {
                                        let interactions = day_info.get("interactions")
                                            .and_then(|v| v.as_u64()).unwrap_or(0);
                                        
                                        let daily_total: u64 = hours.iter()
                                            .filter_map(|v| v.as_u64())
                                            .sum();
                                        ...
                                        weekly_totals.push((day_name, daily_total, daily_avg, interactions));
                                        
                                        let bar_length = (daily_avg / 50).min(20) as usize;
                                        let bar = "█".repeat(bar_length);
                                        
                                        println!("   {} │ Total: {:>5} │ Avg: {:>3} │ Int: {:>2} │ {}", 
                        ...
                        let week_total: u64 = weekly_totals.iter().map(|(_, total, _, _)| *total).sum();
                        let week_avg = if !weekly_totals.is_empty() {
                            week_total / weekly_totals.len() as u64
                        } else { 0 };
                        
                        // Find busiest day
                        if let Some((busiest_day, max_total, _, _)) = weekly_totals.iter()
                            .max_by_key(|(_, total, _, _)| *total) {
                            println!("   🔥 Busiest Day: {} ({} events)", busiest_day, max_total);
                        }
                        
                        println!("   📈 Weekly Summary: {} total events, {} daily average", 
                            week_total, week_avg);
                    }
